from __future__ import annotations

from dataclasses import dataclass
from typing import Dict, Iterable, List, Sequence, Tuple


@dataclass(frozen=True)
class Node:
    id: int
    key: str
    label: str
    x: float
    y: float
    type: str


@dataclass(frozen=True)
class Edge:
    from_id: int
    to_id: int
    cost: float


@dataclass
class CampusGraph:
    nodes_by_id: Dict[int, Node]
    node_id_by_key: Dict[str, int]
    adjacency: Dict[int, List[Tuple[int, float]]]
    edge_costs: Dict[Tuple[int, int], float]

    def neighbors(self, node_id: int, weighted: bool = True) -> Iterable[Tuple[int, float]]:
        for nxt, cost in self.adjacency.get(node_id, []):
            yield (nxt, cost if weighted else 1.0)

    def node_key(self, node_id: int) -> str:
        return self.nodes_by_id[node_id].key

    def node_id(self, node_key: str) -> int:
        return self.node_id_by_key[node_key]

    def edge_cost(self, from_id: int, to_id: int) -> float:
        return self.edge_costs[(from_id, to_id)]

    def weighted_path_cost(self, node_path: Sequence[str]) -> float:
        if len(node_path) < 2:
            return 0.0
        total = 0.0
        for index in range(len(node_path) - 1):
            a = self.node_id(node_path[index])
            b = self.node_id(node_path[index + 1])
            total += self.edge_cost(a, b)
        return total
