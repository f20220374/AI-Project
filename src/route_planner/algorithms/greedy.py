...
def run_greedy(problem: RoutePlanningProblem, heuristic: Heuristic) -> SearchResult:
    """Run Greedy Best-First Search using weighted edges and supplied heuristic."""
    start_time = time.perf_counter()
    start_state = problem.initial_state()

    counter = 0
    frontier: list[tuple[float, int, object]] = [(heuristic(problem, start_state), counter, start_state)]
    parent = {}
    ...
    while frontier:
        ...
        for action, next_state in problem.successors(state, weighted=True):
            ...
            prev_cost = best_g.get(next_state)
            if prev_cost is not None and next_cost >= prev_cost:
                continue
